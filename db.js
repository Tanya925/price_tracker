...
// 判斷目前程式是不是跑在 Azure App Service 上（WEBSITE_SITE_NAME 是 Azure 的環境變數）
const isAzure = !!process.env.WEBSITE_SITE_NAME;

// 判斷目前程式是不是跑在 Render 上（RENDER 是 Render 的環境變數）
const isRender = !!process.env.RENDER;

// 設定資料庫資料夾位置
// Azure 用 /home/data/db
// Render 如果有設定 Persistent Disk()，通常用 /var/data/db（但因為免費板沒有Persistent Disk，所以還是會跳到最下面的那個else）
...
else if (isRender && fs.existsSync('/var/data')) {  // 除了檢查否為 Render，還會檢查 /var/data 這個資料夾是否存在（因為免費板 Render 沒有 Persistent Disk，所以沒有 /var/data 這個資料夾）
  dataDir = '/var/data/db';
} 
else {  // 所以如果是在本機執行，或是使用免費版的 Render，會進到這個 else 裡面，使用專案下的 db 資料夾
